...
val_set = ['val']
d_sets = {'videos_val': val_set, 'videos_train':['train']}
# videos_val = ['MOT17-02-DPM']
# videos_train = ['MOT19-04-DPM','MOT17-05-DPM','MOT17-09-DPM','MOT17-11-DPM','MOT17-13-DPM']
...
def parse_and_sched(dl_dir='.'):
    ...
    for d_set in d_sets:
        for dataset in d_sets[d_set]:
            ...
            if dataset is 'train':
                video_rm = open(os.path.join(dataset_path,dataset,'list0_prohibited_1000.txt'), 'r')
                data = video_rm.readlines()
                video_rm_list = []
                for video_rm in data:
                    video_rm_list.append(video_rm.strip())
            for video in videos:
                if video in ['list.txt','list0_prohibited_1000.txt']:
                    continue
                if dataset is 'train':
                    if video in video_rm_list:
                        continue
                video = dataset+'/'+video
