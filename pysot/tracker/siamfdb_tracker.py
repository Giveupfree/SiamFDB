...
class SiamFDBTracker(SiameseTracker):
    def __init__(self, model, cfg):
        super(SiamFDBTracker, self).__init__()
    ...
    def _convert_cls(self, cls):
        ...
        # cls = cls[:, :, :, :].sigmoid().data[:, 1, :, :].cpu().numpy()
        return cls
    ...
    def track_cam(self, img, hp):
        ...
        outputs = self.model.track(x_crop)
        outcls = outputs['cls'][:, :, :, :][:, 1, :, :]
        cls = outcls.data.cpu().numpy().squeeze()
        ltrbs = outputs['loc'].data.cpu().numpy().squeeze()
        # plt.figure(figsize=(25, 25))
        # plt.imshow(cls,cmap='plasma') # plasma /viridis
        # plt.show()
        score = cls.reshape(-1)
        ltrbs = ltrbs.reshape(4, -1)
        ltrbs[0, :] = self.points[:, 0] - ltrbs[0, :]
