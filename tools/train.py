...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    # win = viz.line(X=np.array([-1]),Y=np.array([0]),opts=dict(title='loss'))
    ...
    for idx, data in enumerate(train_loader):
        if epoch != idx // num_per_epoch + start_epoch:
            ...
            # viz.line([loss.item()], [epoch], win=win,  # win要保持一致
            #          update='append')
        tb_idx = idx
...
def main():
    ...
    if cfg.TRAIN.START_EPOCH >= cfg.BACKBONE.TRAIN_EPOCH:
        for param in dist_model.module.backbone.parameters():
            param.requires_grad = False
        for m in dist_model.module.backbone.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eval()
