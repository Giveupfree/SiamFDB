...
parser.add_argument('--end_epoch', type=int, default=20,
        help='snapshot of models to eval')
parser.add_argument('--snapshot_dir', type=str, default='snapshotGAT',
        help='snapshot of models to eval')

parser.add_argument('--config', type=str, default='./experiments/SiamFDB_r50/config.yaml',
        help='config file')
        ...
        if not os.path.exists(snapshot):
            if idx is not end_epoch:
                ...
                if not os.path.exists(snapshot):
                    print(snapshot + 'file does not exist')
                    continue
                model_name = snapshot.split('/')[-3]
        model = load_pretrain(model, snapshot).cuda().eval()
        # build tracker
        tracker = SiamFDBTracker(model, cfg.TRACK)
        
        model_path = os.path.join('results', args.dataset, model_name + "e{}".format(idx))
